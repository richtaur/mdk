...
function compressVideoFiles (config) {
...
function convertVideoFiles (config) {
...
function gififyVideoFiles (config) {
...
function resize (config, pixelPerfect) {
	...
	for (let i = 3; i < config._.length; ++i) {
		const target = config._[i];
		if (target) {
			resizeVideo(size, target, pixelPerfect);
		} else {
			console.log("- Unsure how to operate on " + target);
		}
	}
...
function resizeVideo (size, target, pixelPerfect) {
	const cleanTarget = clean(target);
	const tmp = clean("tmp_" + target);
	const pixelArgs = (pixelPerfect ? "-sws_flags neighbor " : "");
	const command = `ffmpeg -loglevel panic -i ${cleanTarget} -s ${size} ${pixelArgs}-c:a copy ${tmp} && mv ${tmp} ${cleanTarget}`;
