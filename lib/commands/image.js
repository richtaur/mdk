...
function compressImageFiles (config) {
...
function compress (target) {
	...
	const ffmpegCommand = `ffmpeg -loglevel panic -i ${cleanTarget} -crf 1 ${tmp}; mv ${tmp} ${cleanTarget}`;
...
function resizeImage (size, target, pixelPerfect) {
	const cleanTarget = clean(target);
	let command = `mogrify -resize ${size} ${cleanTarget}`;
	// const mogrifyCommand = `mogrify -filter point -resize ${size} ${cleanTarget}`;
	if (pixelPerfect) {
		const tmp = clean("tmp_" + target);
		command = `ffmpeg -loglevel panic -i ${cleanTarget} -s ${size} -sws_flags neighbor ${tmp} && mv ${tmp} ${cleanTarget}`;
	}

	childProcess.execSync(command);
}

function resizeImages (config, pixelPerfect) {
	...
	for (let i = 3; i < config._.length; ++i) {
		const target = config._[i];
		if (target) {
			resizeImage(size, target, pixelPerfect);
		} else {
			console.log("- Unsure how to operate on " + target);
		}
	}
...
module.exports = function (config) {
	...
	switch (command) {
		...
		case "resize": {
			resizeImages(config, false);
			break;
		}

		case "resize-pixels": {
			resizeImages(config, true);
			break;
		}
