...
function normalize (target) {
	const tmp = "tmp_" + target;
	const ffmpegCommand = `ffmpeg -loglevel panic -i ${target} -af loudnorm=I=-23:LRA=7:tp=-2:print_format=json -f null -\nffmpeg -loglevel panic -i ${target} -af loudnorm=I=-23:LRA=7:tp=-2:measured_I=-30:measured_LRA=1.1:measured_tp=-11:measured_thresh=-40.21:offset=-0.47 -ar 48k -y ${tmp}; mv ${tmp} ${target}`;
	childProcess.execSync(ffmpegCommand);
