...
function compressAudioFiles (config) {
...
function normalizeAudioFiles (config) {
...
function reverseAudioFiles (config) {
	console.log("- Executing audio reversal ...");
...
function reverse (target) {
	const cleanTarget = clean(target);
	const tmp = clean("tmp_" + target);
	const ffmpegCommand = `ffmpeg -loglevel panic -i ${cleanTarget} -vf reverse -af areverse ${tmp}; mv ${tmp} ${cleanTarget}`;
	childProcess.execSync(ffmpegCommand);
	console.log("- reversed " + target);
}

function volumeAudioFiles (config) {
	console.log("- Executing audio volume ...");

	// First arg is the format information
	const volume = config._[2];

	// If args are missing, show help examples
	if (volume === undefined || config.length < 3) {
		help.audioVolume();
		return;
	}
...
function volumeAudioFile (volume, target) {
	const cleanTarget = clean(target);
	const tmp = clean("tmp_" + target);
	const ffmpegCommand = `ffmpeg -loglevel panic -i ${cleanTarget} -filter:a "volume=${volume}" ${tmp}; mv ${tmp} ${cleanTarget}`;
	childProcess.execSync(ffmpegCommand);
	console.log(`- volume set for ${target}`);
}
...
module.exports = function (config) {
	...
	switch (command) {
		...
		case "normalize": {
			normalizeAudioFiles(config);
			break;
		}

		case "reverse": {
			reverseAudioFiles(config);
			break;
		}

		case "volume": {
			volumeAudioFiles(config);
			break;
		}
